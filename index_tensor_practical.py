...
print("index_reduce:\n", torch.index_reduce(torch.zeros(5), 0, torch.tensor([1, 1, 3]), torch.tensor([2.0, 3.0, 4.0]), reduce="amax"))

print("index_select:\n", torch.index_select(torch.tensor([10, 20, 30, 40]), 0, torch.tensor([1, 3])))

print("masked_select:\n", torch.masked_select(x, x > 2))

print("movedim:\n", torch.movedim(torch.ones(2, 3, 4), 0, -1).shape)
...
print("nonzero:\n", torch.nonzero(torch.tensor([0, 1, 2, 0, 3])))

print("permute:\n", torch.ones(2, 3, 4).permute(2, 0, 1).shape)
...
print("select_scatter:\n", torch.select_scatter(torch.zeros(2, 3),src, 1,1))

print("slice_scatter:\n", torch.slice_scatter(torch.zeros(2, 3), torch.tensor([[1., 2., 3.]]), dim=0, start=0, end=1))

print("scatter_add:\n", torch.scatter_add(torch.zeros(2, 4), 1, torch.tensor([[1], [2]]), torch.tensor([[9.0], [10.0]])))

print("scatter_reduce:\n", torch.scatter_reduce(torch.zeros(2, 4), 1, torch.tensor([[1], [2]]), torch.tensor([[9.0], [10.0]]), reduce="sum"))

print("split:\n", torch.split(torch.tensor([1, 2, 3, 4]), 2))
...
print("take:\n", torch.take(torch.tensor([[1, 2], [3, 4]]), torch.tensor([0, 2])))

print("take_along_dim:\n", torch.take_along_dim(torch.tensor([[10, 20], [30, 40]]), torch.tensor([[0, 0], [1, 0]]), dim=1))

print("tensor_split:\n", torch.tensor_split(torch.arange(10), 3))
